...
public sealed partial class TodoServiceTests
{
	...
	public async Task UpdateItemAsyncShouldReturnFailedWhenExceptionIsThrown()
	{
		...
		ErrorOr<Updated> result = await _sut
			.UpdateItemAsync(itemId, request, _cancellationToken)
	...
	public async Task UpdateItemAsyncShouldReturnNotFoundWhenNotFound()
	{
		...
		itemMock.Setup(x => x.GetByIdAsync(itemId, It.Is<Query<ItemEntity>>(q => q.TrackChanges), _cancellationToken))
		...
		AssertionHelper.AssertInScope(() =>
		{
			...
			itemMock.Verify(x => x.GetByIdAsync(itemId, It.Is<Query<ItemEntity>>(q => q.TrackChanges), _cancellationToken), Times.Once);
