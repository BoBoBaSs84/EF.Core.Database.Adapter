...
public sealed partial class TodoServiceTests
{
	...
	public async Task UpdateListAsyncShouldReturnFailedWhenExceptionIsThrown()
	{
		...
		ErrorOr<Updated> result = await _sut
			.UpdateListAsync(listId, request, _cancellationToken)
	...
	public async Task UpdateListAsyncShouldReturnNotFoundWhenNotFound()
	{
		...
		listMock.Setup(x => x.GetByIdAsync(listId, It.Is<Query<ListEntity>>(q => q.TrackChanges), _cancellationToken))
		...
		AssertionHelper.AssertInScope(() =>
		{
			...
			listMock.Verify(x => x.GetByIdAsync(listId, It.Is<Query<ListEntity>>(q => q.TrackChanges), _cancellationToken), Times.Once);
