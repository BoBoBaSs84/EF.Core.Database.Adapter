...
public sealed partial class TodoServiceTests
{
	...
	public async Task GetListAsyncShouldReturnNotFoundWhenNotFound()
	{
		...
		listMock.Setup(x => x.GetSingleAsync(
			It.IsAny<Expression<Func<ListEntity, ListResponse>>>(),
			It.IsAny<Query<ListEntity>>(),
			_cancellationToken)
		...
		AssertionHelper.AssertInScope(() =>
		{
			...
			listMock.Verify(x => x.GetSingleAsync(It.IsAny<Expression<Func<ListEntity, ListResponse>>>(), It.IsAny<Query<ListEntity>>(), _cancellationToken), Times.Once);
